...
globalThis['handleStyles'] = (resetMusic) => {
    // Reset
    Utils.exec(`mkdir -p "${GLib.get_user_config_dir()}/ags/scss"`);
    if (resetMusic) {
        // Utils.exec(`bash -c 'echo "" > ${GLib.get_user_config_dir()}/ags/scss/_musicwal.scss'`); // reset music styles
        Utils.exec(`bash -c 'echo "" > ${GLib.get_user_config_dir()}/ags/scss/_musicmaterial.scss'`); // reset music styles
    }
    // Generate overrides
...
`,
        `${GLib.get_user_config_dir()}/ags/scss/_lib_mixins_overrides.scss`)
    // Compile and apply
    async function applyStyle() {
        Utils.exec(`mkdir -p ${COMPILED_STYLE_DIR}`);
        Utils.exec(`sass -I "${GLib.get_user_config_dir()}/ags/scss" -I "${App.configDir}/scss/fallback" "${App.configDir}/scss/main.scss" "${COMPILED_STYLE_DIR}/style.css"`);
        App.resetCss();
